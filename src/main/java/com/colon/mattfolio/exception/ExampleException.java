...
public class ExampleException extends MasterException {
    ...
    public enum Reason implements BaseReason {
        INVALID_REQUEST(ApiResultCode.Example.INVALID_REQUEST.getCode(), Message.getMessage("example.invalid.request")), // 요청 파라미터 이상
        INVALID_ID(ApiResultCode.Example.INVALID_ID.getCode(), Message.getMessage("example.invalid.id")), // 유효하지 않은 아이디
        NOT_EXIST_NAME(ApiResultCode.Example.NOT_EXIST_NAME.getCode(), Message.getMessage("example.not.exist.name")), // 존재하지 않는 이름
        DUPLICATE_ID(ApiResultCode.Example.DUPLICATE_ID.getCode(), Message.getMessage("example.duplicate.id")); // 중복된 아이디 존재
